...
const opcaoInstrutorSelecionados = JSON.parse(localStorage.getItem('opcaoInstrutorSelecionados') || '[]')
// cria variaveis condicaoCodigo, condicaoCurso e condicaoAluno e selectCriado e atribui o valor false a elas
let condicaoCodigo = false, condicaoCurso = false, condicaoAluno = false, condicaoInstrutor = false, selectCriado = false;
...
function criarSelect() {
    ...
    if (!selectCriado) {
        ...
        // Adiciona atributo a esse select
        select3.setAttribute('aria-label', 'Multiple select example');
        // Adiciona select2 ao selectModalBody
        selectModalBody.appendChild(select3);
        // Cria o elemento option
        const opcao3 = document.createElement('option');
        // Não define opção padrão selecionada
        opcao3.selected = false;
        // Texto da opção
        opcao3.textContent = 'Selecione os Instrutores:';
        // Adiciona opcao2 ao select2
        select3.appendChild(opcao3);
        // Cria no select todas as opções de nomeAluno
        for (let indice = 0; indice < nomeInstrutor.length; indice++) {
            const opcao = document.createElement('option');
            opcao.value = nomeInstrutor[indice];
            opcao.textContent = nomeInstrutor[indice]
...
function adicionarOpcaoInstrutor(){
    const nomeDosInstrutores = document.getElementById('nomeDosInstrutores')
    const opcaoSelecionada = nomeDosInstrutores.selectedOptions

    for(let indice = 0; indice < opcaoSelecionada.length; indice++){
        const opcao = opcaoSelecionada[indice]

        if(opcao.value === 'Selecione os Instrutores:'){
            condicaoInstrutor = false;
            if(condicaoInstrutor === false){
                alert('Por favor selecione um ou mais instrutores')
            }
        }else{
            condicaoInstrutor = true
            if(condicaoInstrutor === true){
                opcaoInstrutorSelecionados.push(`${indice+1}-${opcao.value}`)
            }
        }
    }
    document.getElementById('nomeDosInstrutores').value = 'Selecione os Instrutores:'
}
...
function cadastroTurma() {
    ...
    if (condicaoCodigo === true && condicaoCurso === true && condicaoAluno === true && condicaoInstrutor === true) {
...
function salvarOpcaoInstrutor(){
    localStorage.setItem('opcaoInstrutorSelecionados',JSON.stringify(opcaoInstrutorSelecionados))
}
...
console.log(`Opção Instrutor Selecionados: ${opcaoInstrutorSelecionados}`)
